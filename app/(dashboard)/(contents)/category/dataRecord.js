...
import CategoryEditingModal from './categoryEditingModal';
import CategoryRemovingModal from './categoryRemovingModal';
  ...
  return (
    <>
      <tr className="text-sm whitespace-nowrap">
        {/* Serial */}
        <td className="p-4 border-b-[1px] border-slate-100">{index + 1}</td>
        {Children.toArray(
          tableHeader.map((col) => {
            if (col.value)
              switch (col.value) {
